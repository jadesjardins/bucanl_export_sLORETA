...
%
% Usage:
%   >>  com= pop_writesLORdat(EEG, outfname, outfpath, optstr);
%
% Inputs:
...
function com = pop_writesLORdat(EEG, outfname, outfpath, optstr)
...
if nargin < 3
    ...
    {...
        {'Style', 'text', 'string', 'Enter sLORETA data export parameters.', 'FontWeight', 'bold'}, ...
     ...
     'pophelp(''pop_writesLORdat'');', 'Select sLORETA export parameters -- pop_writesLORdat()' ...
     );
...
if isempty(optstr);
    com = sprintf('writesLORdat( %s, ''%s'', ''%s'');', inputname(1), outfname, outfpath);
else
    com = sprintf('writesLORdat( %s, ''%s'', ''%s'', %s );', inputname(1), outfname, outfpath, optstr);
end
